...
describe("`let` restricts the scope of the variable to the current block - ", () => {
  describe("`let` vs. `var`.", () => {
    it("`var` works as usual, it does not restrict scope", () => {
      var varX = false;
      if (true) {
        var varX = true; // we override the initial varX
      }
    ...
    it("`let` restricts scope to inside the block", () => {
      let letX = false;
      if (true) {
        let letX = true; // let letX stays within the if statement
      }
      expect(letX).toBe(false);
    });

    it("`var` does not restrict scope to inside the block in `for` loops", () => {
      var counter = 100; // eventually this one gets discarded
      for (var counter = 1; counter < 50; counter++) {} // Here the var will overrite the counter
      expect(counter).toBe(50);
    });

    it("`let` restricts scope to inside the block also in `for` loops", () => {
      let counter = 100;
      for (let counter = 1; counter < 50; counter++) {} // the let counter variable will stay WITHIN the for loop
...
describe("`const` is like `let` plus read-only. ", () => {
  describe("scalar values are read-only", () => {
    it("number are read-only", () => {
      const constNum = 0;
      // constNum = 1; //if uncommented recieve error "Assignment to constant variable"
      expect(constNum).toBe(0);
    });

    it("string are read-only", () => {
      const constString = "I am a const";
      // constString = "Cant change you?";  //if uncommented recieve error "Assignment to constant variable"
  ...
  describe("complex types are NOT fully read-only", () => {
    it("arrays is not fully read-only", () => {
      const arr = [42, 23];
      arr[0] = 0; // even though its a const variable, you can still change the array internally
...
describe("`string.includes()` finds string within another string. ", () => {
  describe("find a single character", function () {
    ...
    it("reports false if character was not found", function () {
      const expected = false; // "abc" is not in "xyz"
  ...
  describe("search for an empty string, is always true", function () {
    ...
    it("in `abc`", function () {
      const x = ""; // includes will be true if you search for an empty string
  ...
  describe("takes a position from where to start searching", function () {
    it("does not find `a` after position 1 in `abc`", function () {
      const position = 1; // you can search AFTER a specific position in a string
...
describe("destructuring arrays makes shorter code. ", () => {
  ...
  it("leading commas", () => {
    ...
    const [, , z] = all; // add some commas to skip the first values
